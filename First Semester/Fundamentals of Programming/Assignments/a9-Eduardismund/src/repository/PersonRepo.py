...
class PersonRepo:
    ...
    def get_person_person(self, id):
        """
        checks whether a person's ID is found in the list or not
        """
        if id in self._data.values():
            return self._data[id]
        else:
            return "Not an existing person"
    ...
    def update(self, person_id, new_person):
        ...
        if person_id in self._data:
            ...
            new_dict = {new_person._id if old_key == person_id else old_key: value for old_key, value in self._data.items()}
            self._data=new_dict
            for i in new_dict:
                print(i, person_id)
        else:
            print(type(person_id), type(new_person))
