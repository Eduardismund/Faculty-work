...
void Test::testDynamicVector() {
    ...
    v.removeElementFromPosition(3);
    assert(v.getSize() == 4);
    assert(v[3] == 4);

    assert(*v.findElement(1) ==1);
    assert(v.findElement(5) == nullptr);
...
void Test::testShelterRepository() {
    ...
    assert(repository.getDogs().getSize() == 1);
    ...
    assert(repository.getDogs().getSize() == 0);
}
...
void Test::testAdminService() {
    ...
    assert(service.getDogs().getSize() == 1);
    try{
        service.addDog("Poodle", "Luna", "https://i.pinimg.com/564x/0d/23/62/0d236297adb04e3d6acaa180307a5a5a.jpg", 5);
...
void Test::testAdoptedRepo() {
    ...
    assert(adoptedRepo.getAdoption().getSize() == 1);
...
void Test::testUserService() {
    ...
    UserService userService(shelterRepository, adoptedRepo);
    assert(userService.getDogShelter().getSize() == 4);

    userService.adoptDog("Luna");
    userService.adoptDog("Cooper");

    assert(userService.getDogsAdopted().getSize() == 2);
    assert(userService.getDogShelter().getSize() == 2);
    ...
    DynamicVector<Dog> filteredDogs1 = userService.getDogsFiltered("Shih Tzu", 10);
    assert(filteredDogs1.getSize() == 1);
    assert(filteredDogs1[0].getName() == "Bailey");

    DynamicVector<Dog> filteredDogs2 = userService.getDogsFiltered("Shih Tzu", 4);
    assert(filteredDogs2.getSize() == 0);
//    assert(filteredDogs1[0].getName() == "Bailey");

    DynamicVector<Dog> filteredDogs3 = userService.getDogsFiltered("", 10);
    assert(filteredDogs3.getSize() == 2);
